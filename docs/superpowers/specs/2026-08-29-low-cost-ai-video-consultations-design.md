...
`HOSTED_AGENT_V1` remains the default. Self-host selection additionally requires `VIDEO_INTERPRETATION_SELF_HOST_APPROVED=true` and `VIDEO_INTERPRETATION_RUNTIME_PROFILE=SELF_HOSTED_AGENT`, while the global media code gate still prevents patient audio. These switches do not replace the measured purchase trigger, soak, contracts, or final reviewed code change that opens the media gate.
