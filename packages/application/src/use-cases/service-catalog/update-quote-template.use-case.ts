...
export class UpdateQuoteTemplateUseCase {
  ...
  async execute(
    ...
    if (actor.role === 'HOSPITAL' && actor.hospitalId !== template.hospitalId) {
      throw new ForbiddenError('Hospital users can only update their own templates');
    }
    if (actor.role === 'PATIENT') {
      throw new ForbiddenError('Patients cannot update quote templates');
    }
    ...
    template.update({
      ...
      defaultValidDays: input.defaultValidDays,
    });
