...
Implement host registration, bearer rotation, job polling, heartbeats, leases, takeover, and capacity advertisement only when moving to `SELF_HOSTED_AGENT`. At that point, use a random per-host 256-bit bearer stored as a digest by the API, no database route from the agent, 30-second leases with 10-second heartbeats, and conditional mutations bound to host and lease version. A separate API-host reconciler runs every two seconds; claim and STOP requests only persist state and never race it by issuing LiveKit cleanup themselves. On expiry it first fences the database execution/capability/tracks/provider input, then outside the transaction revokes and removes the exact old LiveKit identity, and finally waits for the provider closure/expiry/transfer fence before making a new execution claimable. Duplicate audio or billing is less acceptable than a short AI interruption.

Self-hosting the **agent** does not self-host the LiveKit media plane. This Phase D profile requires LiveKit Cloud's `RemoveParticipant` token-revocation behavior, an explicit de-identified capability probe, a `wss://*.livekit.cloud` endpoint, and `VIDEO_INTERPRETATION_LIVEKIT_CLOUD_REVOCATION_VERIFIED=true`. A new Self-hosted job has a server-owned 60-second claim deadline; the host cannot set or renew it, and claim locks the same job row as the scheduler. An expired or unauthorized `DISPATCHING` job is finalized without LiveKit calls only when the database proves no host, lease, capability, dispatch, provider session, or source track ever existed; any violated invariant goes through conservative `STOPPING` cleanup. After an old execution is fully revoked and provider-clean, every `READY_FOR_CLAIM` takeover receives a fresh database deadline of `min(now + 60 seconds, original application deadline)` in the same transaction as its new identity; `started_at` is never reset, and an exhausted or missing application cutoff terminates instead of becoming claimable. The initial exact-room join JWT and every renewed lease expire no later than both the 30-second lease window and the server-owned maximum-AI application deadline, but TTL is only defense in depth because connected clients can receive refreshed tokens. Every execution uses a new identity. Lease loss, application-deadline expiry, STOP, consultation close, host credential rotation/revocation, release/allowlist expiry or revocation, and budget exhaustion all first persist the old exact identity in durable `STOPPING` cleanup state. The independent scheduler checks the deadline even if a faulty Agent keeps sending heartbeats; heartbeats at or after the deadline are rejected. The reconciler calls removal with a near-current `revokeTokenTs` even if that identity is already offline, then verifies the identity is absent; failure remains retryable and cannot become a terminal status or admit the same job execution again. Isolation is per job/execution, so unrelated rooms do not block one another. Only after token/identity cleanup and provider-fence finality may the job become `DISPATCHING`, `STOPPED`, or `BUDGET_EXHAUSTED` as appropriate. A future self-hosted LiveKit server requires a separately reviewed revocation or room-isolation design; short TTL and ordinary participant removal are insufficient.
...
- `medora-video-interpretation-reconcile` (mandatory for Hosted and Self-hosted profiles; two-second cadence on the existing API host);
- `medora-video-cleanup-worker`;
- `medora-video-ai-dispatch-worker`.

They are processes, not separate servers. Each unit has restart limits, structured redacted logs, and a dedicated database role where practical. The lifecycle reconciler records lease ownership separately from pass outcomes: only a recent fully successful pass, with no newer failed pass, satisfies the START health gate. A renewed lease or an `alreadyRunning` response never counts as health, and any incomplete pass, retryable remote cleanup failure, or unhandled exception records a content-free failure outcome.

`medora-video-interpretation-reconcile` runs three independent loops: Hosted lifecycle, Self-hosted database fence, and Self-hosted remote cleanup. Thus neither a slow Hosted call nor a slow identity revocation can delay the two-second self-host lease fence. Each profile acquires a PostgreSQL durable runner lease with a unique owner, renews it before bounded groups of LiveKit calls, observes a 45-second pass budget, and releases it in `finally`; a competing API replica returns `alreadyRunning`, while a crashed owner becomes replaceable after the lease expires. The worker's HTTP timeout exceeds the pass budget. No business transaction is held across LiveKit network calls. The loops check server-owned dispatch/bootstrap/application deadlines, exact approval/allowlist authority, and hard budget usage. Hosted invalidation persists `STOPPING`, then idempotently deletes and verifies absence of the exact dispatch, revokes/removes/verifies the exact Agent identity, and waits for the provider fence. Cleanup-pending jobs continue to consume one of the two global slots and retain their full budget reservation. Only verified terminal cleanup releases capacity and reduces the reservation to reconciled consumption. This prevents two naturally expired or never-bootstrapped Hosted jobs from permanently exhausting V1 capacity without requiring another server.

Before `createDispatch`, the API stores a random non-PHI correlation key, exact job/room/generation/execution/identity envelope, request timestamp, and stage deadline; the same envelope is placed in dispatch metadata. Every interpretation LiveKit control client explicitly disables SDK cross-region failover, because replaying non-idempotent dispatch creation could create two agents. Even after `createDispatch` returns, its ID is only a candidate: pending remains durable until `listDispatch(room)` finds exactly one full-envelope match with that same ID. Zero, duplicate, ID-mismatch, timeout, and network-rejection outcomes fail closed without releasing the reservation or capacity. A scheduler recovering an unknown create outcome never retries creation. It lists the room and touches only exact full-envelope matches: one is adopted only if the job was not already fenced, multiple exact duplicates are all deleted, and malformed/foreign dispatches are ignored. Zero matches remain capacity/reservation blocking unless two separated authoritative absence observations occur after a separately executable-tested LiveKit late-commit/list-consistency bound. That bound is not yet proved, so the non-overridable `HOSTED_DISPATCH_ABSENCE_BOUND_VERIFIED` gate remains false and Hosted START remains unqualified. This is deliberate fail-closed code readiness, not a production claim.

Both Hosted and optional Self-hosted central-agent profiles also require the same verified LiveKit Cloud media-plane boundary: the effective endpoint must be `wss://*.livekit.cloud` and `VIDEO_INTERPRETATION_LIVEKIT_CLOUD_REVOCATION_VERIFIED=true` must attest the de-identified cutoff/rejoin probe. Hosted START needs this gate in addition to dispatch-absence qualification. A requested Self-host profile that fails its approval or media-plane gate returns disabled instead of silently falling back to Hosted. This common requirement exists because both cleanup paths rely on token cutoff plus exact-identity removal to prevent an old execution from rejoining with an original or server-refreshed token.
...
Hosted deployment is incomplete unless `medora-video-interpretation-reconcile.service` is installed and enabled on the existing API Lightsail. It is the only caller of Hosted unknown-dispatch recovery, bootstrap/application deadline fencing, exact dispatch/identity cleanup, and final budget/capacity release. START fails closed unless every required profile completed a fully successful pass within 90 seconds and has no newer failed pass; lease heartbeats alone do not qualify. Follow `apps/interpretation-agent/deploy/HOSTED.md`; this service is another process on the existing host, not another Lightsail.
...
The host does not hold the LiveKit server API secret. It requests a lease-bounded, generation/execution-bound agent participant token from `apps/api` after a valid job claim. LiveKit Cloud remains the SFU and supplies the server-side token-revocation boundary described in §7.3. The independent reconciler runs on the existing API Lightsail, not on the optional interpretation host, so a hung interpretation process cannot suppress expiry cleanup. If local LiveKit Turn Detector `v1-mini` is used, soak-test CPU and inference timeouts: LiveKit recommends compute-optimized rather than burstable hosts for that model, so the standard `$24` Lightsail bundle is not assumed adequate.
...
- install and enable the mandatory lifecycle reconciler on the existing API Lightsail; do not launch Hosted evaluation without its health check;
...
Implementation status on 2026-08-30: the repository contains the dedicated WebSocket adapter, source/translated caption parsing, translated PCM buffering, server-listed microphone reconciliation, watchdog-bound explicit subscriptions, two-session cap, per-turn `session.close`/`session.closed` finality, target-language LiveKit audio publication, and a synthetic/de-identified capability probe. The pinned OpenAI exact-endpoint probe has returned both source/translated transcripts and translated PCM, and the named Hosted Agent is running in the isolated LiveKit project. The media-adapter code gate is now `true`, but that fact alone authorizes no runtime request: feature, provider, release-approval, allowlist, consent, classification, budget, capacity, and runtime gates remain mandatory.

The staging API has a separate `DEIDENTIFIED_EVALUATION` E2E mode that requires both `VIDEO_INTERPRETATION_DEPLOYMENT_TIER=STAGING` and `VIDEO_INTERPRETATION_DEIDENTIFIED_E2E_ENABLED=true`. It permits at most one global AI room and rejects an AI duration above 300 seconds. The waiver is derived only after the server loads a business-field-empty consultation whose random room prefix, `synthetic: true` metadata, de-identified classification, and at-most-30-minute expiry match the synthetic authority contract; the same predicate is rechecked under the START row lock. In that narrow mode, an unverified Hosted dispatch absence bound or LiveKit Cloud token-revocation proof does not falsely become “verified”; instead, ambiguous creation stays capacity-blocking, the agent/application deadline is fixed, and the reconciler remains mandatory. `REAL_PATIENT` still fails the non-overridable code gate before dispatch. The loopback-only two-browser harness in `apps/api/deploy/video-staging-e2e-harness.mjs` creates only synthetic authority rows, short-lived room tokens, a $0.50 per-room reservation ceiling, and a one-consultation `SYNTHETIC_E2E` approval. That approval records privacy, observability, and retention attestations as `false`; it is authorized only by the synthetic predicate and cannot be reused as a normal release approval. A supervised human room test of captions, translated audio, interruption, STOP, and cleanup is still required before Phase B is called complete.
...
Code-readiness status on 2026-08-29: the control plane now requires a current release approval plus consultation allowlist, pins the exact provider model/endpoint/rate, reserves two provider streams with a safety margin, enforces room/daily/monthly limits, and reconciles a hard job budget from server timestamps on every watchdog refresh. Approval revocation synchronously invalidates capabilities, tracks, provider input, and queued output. The independent lifecycle reconciler also fences Hosted jobs after dispatch/bootstrap/application deadline, approval/allowlist expiry or replacement, and hard-budget exhaustion when no Agent callback arrives. Exact dispatch/identity cleanup and provider finality are durable prerequisites for releasing the global slot and unused reservation. Unknown dispatch-creation outcomes use full-envelope correlation recovery and remain blocked when authoritative remote absence cannot yet be proved. The client has separate original/translated audio toggles, reliable content-free playout boundaries for optional ducking, and an audited human-interpreter escalation request while the original call remains active.

This is not a contracted real-patient launch. `VIDEO_INTERPRETATION_REAL_PATIENT_RELEASE_IMPLEMENTED` remains `false`, and the only runtime-allowlisted adapter is still the dedicated `/v1/realtime/translations` profile that this document does not treat as PHI-eligible. The approval API rejects that exact endpoint for `REAL_PATIENT`; contracts, project settings, provider qualification, a covered endpoint adapter, and executable privacy evidence remain external blocking gates.

### Phase D: Optional self-hosted migration

Provision one 4 GB Lightsail only after a measured hosted-cost, runtime, portability, or contractual trigger. First prove that the bundle can run the selected media pipeline and local Turn Detector without CPU-credit timeouts; otherwise choose suitable compute rather than forcing the `$24` target. Implement host secrets, leases, heartbeat, rebuild, and takeover only in this phase. The supplied V1 supervisor is intentionally limited to one concurrent AI room per registered host. Add a second host only after self-hosting is selected and a second simultaneous room or warm standby is actually required; lifting the per-host limit instead requires a separately reviewed concurrent supervisor and multi-room soak.

Code-readiness status on 2026-08-29: no instance has been provisioned. The repository now includes admin-only host registration/rotation/revocation, digest-only bearer storage, server-selected job claims with a 60-second acquisition deadline, a lease-bounded exact-room LiveKit token, job capabilities bound to the host credential version and 30-second lease, 10-second heartbeats, and an optional single-job RTC supervisor that has no LiveKit server secret or PostgreSQL route. Registration and the database reject per-host capacity above one until concurrent supervision is implemented and soaked. A separate two-second reconciler service runs on the existing API host, with a cross-process durable runner lease and pass deadline; claim and STOP paths do not issue competing cleanup calls. Every self-hosted execution invalidation first fences the database authority and retains the old exact identity; the reconciler then revokes cached/refreshed tokens and removes that identity through LiveKit Cloud even if it is currently offline. Never-claimed jobs time out and release their reservation instead of permanently occupying one of the two slots. Takeover or a terminal stop becomes possible only after revocation/removal and provider closure or conservative expiry, and a new START is rejected while cleanup remains pending. Both systemd examples use `LoadCredential=` with root-owned secret files and document immutable secret-free rebuilds and the no-snapshot rule. Self-host claim stays closed until the Cloud revocation probe is explicitly attested.

`HOSTED_AGENT_V1` remains the default. Self-host selection additionally requires `VIDEO_INTERPRETATION_SELF_HOST_APPROVED=true`, `VIDEO_INTERPRETATION_RUNTIME_PROFILE=SELF_HOSTED_AGENT`, and the explicit LiveKit Cloud revocation qualification. The implemented media adapter does not authorize the self-hosted profile, PHI, or real-patient use by itself. These switches do not replace the measured purchase trigger, soak, contracts, project settings, or the separate real-patient code gate.
...
- LiveKit Cloud revocation is probed with an offline old identity: both its original JWT and a server-refreshed JWT fail to rejoin after `RemoveParticipant(..., { revokeTokenTs })`; selecting a self-hosted LiveKit media server fails closed;
- the independent two-second API-host reconciler fences an expired lease without any replacement claim, retries cleanly after crashes before/after removal, and leaves `STOPPING` blocked on LiveKit errors or a non-terminal provider fence;
- two concurrent reconcilers produce one effective fence; a late heartbeat, old capability, old lease version, and old execution all fail, while a timely 10-second heartbeat does not trigger removal;
- takeover uses a never-reused execution identity, waits for old-token revocation/identity absence and the provider closure/expiry/transfer fence, and removes no human participant or original human audio;
