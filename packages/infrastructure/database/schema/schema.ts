...
export const caseTreatmentStage = pgEnum("CaseTreatmentStage", ['INTAKE', 'CONFIRMED', 'IN_TREATMENT', 'POST_TREATMENT', 'COMPLETED', 'FOLLOW_UP'])
...
export const conversations = pgTable("conversations", {
	...
	uniqueIndex("conversations_hospital_patient_case_hospital_unique").using("btree", table.caseId.asc().nullsLast().op("uuid_ops"), table.hospitalId.asc().nullsLast().op("uuid_ops")).where(sql`${table.category} = 'HOSPITAL_PATIENT' and ${table.caseId} is not null and ${table.hospitalId} is not null`),
	...
	uniqueIndex("conversations_hospital_patient_case_hospital_unique_idx").using("btree", table.caseId.asc().nullsLast().op("uuid_ops"), table.hospitalId.asc().nullsLast().op("uuid_ops")).where(sql`${table.category} = 'HOSPITAL_PATIENT' and ${table.caseId} is not null and ${table.hospitalId} is not null`),
	foreignKey({
			columns: [table.caseId],
			foreignColumns: [cases.id],
...
export const messages = pgTable("messages", {
	...
	uniqueIndex("messages_conversation_client_message_id_idx").using("btree", table.conversationId.asc().nullsLast().op("uuid_ops"), table.clientMessageId.asc().nullsLast().op("text_ops")).where(sql`${table.clientMessageId} IS NOT NULL`),
...
export const translationTasks = pgTable("translation_tasks", {
	...
	chunkKey: text("chunk_key").default('default').notNull(),
...
export const aiChatSessions = pgTable("ai_chat_sessions", {
	...
	status: varchar({ length: 20 }).default('ACTIVE').notNull(),
	automationMode: text("automation_mode").default('mechanical').notNull(),
	conditionStatus: varchar("condition_status", { length: 20 }).default('unknown').notNull(),
	...
	journeyCurrentStage: text("journey_current_stage"),
	journeyCurrentPhase: text("journey_current_phase"),
	...
	lastAssistantMessageAt: timestamp("last_assistant_message_at", { withTimezone: true, mode: 'string' }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	...
	index("ai_chat_sessions_automation_mode_idx").using("btree", table.automationMode.asc().nullsLast().op("text_ops"), table.updatedAt.desc().nullsFirst().op("timestamptz_ops")),
...
export const aiChatMessages = pgTable("ai_chat_messages", {
	...
	secondaryAction: varchar("secondary_action", { length: 50 }),
	responseMode: varchar("response_mode", { length: 40 }),
	citations: jsonb().default([]).notNull(),
	metadata: jsonb().default({}).notNull(),
	reasonCodes: jsonb("reason_codes").default([]).notNull(),
	shortlist: jsonb("shortlist").default([]).notNull(),
	writebackStatus: varchar("writeback_status", { length: 20 }).default('pending').notNull(),
	toolTrace: jsonb("tool_trace").default([]).notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	...
	index("ai_chat_messages_role_created_at_idx").using("btree", table.role.asc().nullsLast().op("text_ops"), table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
	index("ai_chat_messages_next_action_idx").using("btree", table.nextAction.asc().nullsLast().op("text_ops")),
]);

export const aiUserProfiles = pgTable("ai_user_profiles", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	patientId: uuid("patient_id").references(() => users.id, { onDelete: 'set null', onUpdate: 'cascade' }),
	anonymousKey: varchar("anonymous_key", { length: 255 }),
	conditionOrGoal: text("condition_or_goal"),
	conditionCategory: varchar("condition_category", { length: 50 }),
	preferredDestination: jsonb("preferred_destination").default([]).notNull(),
	preferredLanguage: varchar("preferred_language", { length: 20 }),
	budgetBand: varchar("budget_band", { length: 20 }),
	urgencyLevel: varchar("urgency_level", { length: 20 }),
	existingReportsStatus: varchar("existing_reports_status", { length: 20 }).default('none').notNull(),
	objectionTags: jsonb("objection_tags").default([]).notNull(),
	leadStage: varchar("lead_stage", { length: 20 }).default('browsing').notNull(),
	nextBestAction: varchar("next_best_action", { length: 50 }),
	memorySummary: text("memory_summary").default('').notNull(),
	sourceConfidenceMap: jsonb("source_confidence_map").default({}).notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	uniqueIndex("ai_user_profiles_patient_id_key").using("btree", table.patientId.asc().nullsLast().op("uuid_ops")),
	uniqueIndex("ai_user_profiles_anonymous_key_key").using("btree", table.anonymousKey.asc().nullsLast().op("text_ops")),
	index("ai_user_profiles_lead_stage_idx").using("btree", table.leadStage.asc().nullsLast().op("text_ops")),
	index("ai_user_profiles_updated_at_idx").using("btree", table.updatedAt.desc().nullsFirst().op("timestamptz_ops")),
]);

export const aiChatTimelineEvents = pgTable("ai_chat_timeline_events", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	sessionId: uuid("session_id").notNull().references(() => aiChatSessions.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
	patientId: uuid("patient_id").references(() => users.id, { onDelete: 'set null', onUpdate: 'cascade' }),
	...
	index("ai_chat_timeline_events_session_id_idx").using("btree", table.sessionId.asc().nullsLast().op("uuid_ops"), table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
	index("ai_chat_timeline_events_event_type_idx").using("btree", table.eventType.asc().nullsLast().op("text_ops"), table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
	index("ai_chat_timeline_events_patient_id_idx").using("btree", table.patientId.asc().nullsLast().op("uuid_ops"), table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
]);

export const aiFollowupTriggers = pgTable("ai_followup_triggers", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	sessionId: uuid("session_id").notNull().references(() => aiChatSessions.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
	patientId: uuid("patient_id").references(() => users.id, { onDelete: 'set null', onUpdate: 'cascade' }),
	...
	channel: varchar({ length: 20 }).default('crm_queue').notNull(),
	reason: text().notNull(),
	payload: jsonb().default({}).notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	resolvedAt: timestamp("resolved_at", { withTimezone: true, mode: 'string' }),
}, (table) => [
	index("ai_followup_triggers_status_due_at_idx").using("btree", table.status.asc().nullsLast().op("text_ops"), table.dueAt.asc().nullsLast().op("timestamptz_ops")),
	index("ai_followup_triggers_trigger_status_idx").using("btree", table.triggerType.asc().nullsLast().op("text_ops"), table.status.asc().nullsLast().op("text_ops")),
	index("ai_followup_triggers_patient_due_at_idx").using("btree", table.patientId.asc().nullsLast().op("uuid_ops"), table.dueAt.asc().nullsLast().op("timestamptz_ops")),
	uniqueIndex("ai_followup_triggers_pending_unique_idx").on(table.sessionId, table.triggerType).where(sql`${table.status} = 'pending'`),
]);

export const aiHandoffs = pgTable("ai_handoffs", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	sessionId: uuid("session_id").notNull().references(() => aiChatSessions.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
	patientId: uuid("patient_id").references(() => users.id, { onDelete: 'set null', onUpdate: 'cascade' }),
	supportTicketId: uuid("support_ticket_id").references(() => supportTickets.id, { onDelete: 'set null', onUpdate: 'cascade' }),
	handoffType: varchar("handoff_type", { length: 40 }).notNull(),
	priority: varchar({ length: 20 }).notNull(),
	reasonCode: varchar("reason_code", { length: 60 }).notNull(),
	brief: jsonb().default({}).notNull(),
	status: varchar({ length: 20 }).default('requested').notNull(),
	assignedTo: uuid("assigned_to").references(() => users.id, { onDelete: 'set null', onUpdate: 'cascade' }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	completedAt: timestamp("completed_at", { withTimezone: true, mode: 'string' }),
}, (table) => [
	index("ai_handoffs_status_priority_idx").using("btree", table.status.asc().nullsLast().op("text_ops"), table.priority.asc().nullsLast().op("text_ops"), table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
	index("ai_handoffs_handoff_type_idx").using("btree", table.handoffType.asc().nullsLast().op("text_ops"), table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
	index("ai_handoffs_patient_id_idx").using("btree", table.patientId.asc().nullsLast().op("uuid_ops"), table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
]);
