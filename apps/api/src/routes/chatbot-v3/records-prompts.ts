...
export function buildRecordsMinimalTriagePrompt(task: RecordsWorkerTask): string {
  ...
  return [
    ...
    'If the user already gave a clear symptom or diagnosis, do not ask them to restate the main problem.',
    'Answer with a short, condition-specific acknowledgement before asking the smallest missing detail.',
    'Never use vague intake-only wording such as "answer this brief question", "state the main problem", or "single main problem" when the symptom is already clear.',
    ...
    '- "followUp": one string that naturally acknowledges what the user already shared and asks only the missing detail',
